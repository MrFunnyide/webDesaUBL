<template>
   <div id="kekayaanAlam" class="p-5">
      <h3 class="text-center">Kekayaan Alam</h3>
      <p class="text-center">Kekayaan Alam Desa Umbul Limus</p>
      <br>
      <div class="d-flex justify-content-around m-3 text-center mobile-res">
         <div class="card" style="width: 16rem;">
            <img src="../assets/image/kekayaanAlam/coklat.jpg" class="card-img-top" alt="coklat">
            <div class="card-body">
               <h5 class="card-title">Coklat</h5>
               <p class="card-desc">Tanaman yang menghasilkan biji kakao yang digunakan untuk membuat coklat.</p>
            </div>
         </div>
         <div class="card" style="width: 16rem;">
            <img src="../assets/image/kekayaanAlam/pisang.jpg" class="card-img-top" alt="pisang">
            <div class="card-body">
               <h5 class="card-title">Pisang</h5>
               <p class="card-desc">Pisang merupakan salah satu buah yang banyak di tanam di Desa Umbul Limus, pisang
                  mempunyai banyak manfaat.</p>
            </div>
         </div>
         <div class="card" style="width: 16rem;">
            <img src="../assets/image/kekayaanAlam/kelapa.jpg" class="card-img-top" alt="kelapa">
            <div class="card-body">
               <h5 class="card-title">Kelapa</h5>
               <p class="card-desc">Kelapa merupakan salah satu buah yang banyak di tanam dan di budidayakan di desa umbul
                  limus. dan kelapa ini biasanya di manfaatkan untuk membuat minyak kelapa.</p>
            </div>
         </div>
         <div class="card" style="width: 16rem;">
            <img src="../assets/image/kekayaanAlam/padi.jpg" class="card-img-top" alt="padi">
            <div class="card-body">
               <h5 class="card-title">Padi</h5>
               <p class="card-desc">Padi adalah tanaman pangan yang merupakan sumber utama beras, salah satu makanan pokok
                  yang paling penting di dunia, terutama di banyak negara di Asia.</p>
            </div>
         </div>
      </div>
   </div>
</template>
<style>
.card-title {
   font-size: 16px;
   font-weight: bolder;
}

.card-desc {
   font-size: 14px;
}
</style>